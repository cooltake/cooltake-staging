...
<div class="ctp-card">
  ...
  <div class="ctp-row">
    ...
    <div style="display:flex;align-items:center;gap:10px;">
      <input id="ctp8Bpm" type="range" min="40" max="240" value="90" oninput="ctp8Slide()" style="flex:1;">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      ...
      <div style="display:flex;gap:6px;">
        <select id="ctp8KeyRoot" class="ctp-select" onchange="ctp8ApplyIfPlaying()">
          <option>C</option><option>C#</option><option>D</option><option>D#</option>
          <option>E</option><option>F</option><option>F#</option><option>G</option>
          <option>G#</option><option>A</option><option>A#</option><option>B</option>
        </select>
        <select id="ctp8KeyMode" class="ctp-select" onchange="ctp8ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      ...
      <input id="ctp8MetroVol" type="range" min="0" max="100" value="80" oninput="ctp8ApplyVolume()">
    </div>
    <div class="ctp-row">
      <label>コード音量</label>
      <input id="ctp8ChordVol" type="range" min="0" max="100" value="80" oninput="ctp8ApplyVolume()">
    </div>
  </div>
  ...
  <div id="ctp8ProgressionSeq" style="display:flex;flex-wrap:wrap;gap:10px;margin:12px 0;"></div>
...
function ctpActualChordName(chordDef, keyRoot, isMinor) {
  ...
  function ctp8LoadShifts() {
    try {
      var raw = localStorage.getItem('cooltake_ctp8_voicing_ops_cliche');
      ctp8VoicingOps = raw ? JSON.parse(raw) : {};
    } catch (e) { ctp8VoicingOps = {}; }
  }
  function ctp8SaveShifts() {
    try { localStorage.setItem('cooltake_ctp8_voicing_ops_cliche', JSON.stringify(ctp8VoicingOps)); } catch (e) {}
  }
  function ctp8OpsKey(chordIdx) { return chordIdx; }
  function ctp8GetOps(chordIdx) { return ctp8VoicingOps[chordIdx] || []; }

  function ctp8ChordAvg(notes) { return notes.reduce(function(a,b){return a+b;}, 0) / notes.length; }
  ...
  function ctp8ClosestOctave(note, prevUpperNotes) {
    var MARGIN = 3;
  ...
  function ctp8VoiceLedBaseNotes(keyRootSemitone, isMinor) {
    var result = [];
    var prevUpper = null;
    ctp8PROGRESSION.chords.forEach(function (chordDef) {
  ...
  function ctp8GetOptions() {
    var idx;
    var octaveShifts = [];
    var voicingOps = ctp8PROGRESSION.chords.map(function (_, i) { return ctp8GetOps(i); });
    return {
  ...
  function ctp8RenderSequence(activeIdx) {
    ...
    wrap.innerHTML = ctp8PROGRESSION.chords.map(function (c, i) {
      ...
      return '<div style="padding:8px 10px;border-radius:10px;min-width:120px;' +
        ...
        '<button class="rate-btn" onclick="ctp8VoicingOp(' + i + ',\'high\')" style="flex:1;padding:4px 2px;font-size:.65rem;" title="最高音を1オクターブ下げる">最高音↓</button>' +
        '<button class="rate-btn" onclick="ctp8VoicingOp(' + i + ',\'up\')" style="flex:1;padding:4px 2px;font-size:.65rem;" title="最低音を1オクターブ上げる">最低音↑</button>' +
        '</div>' +
        '<button class="rate-btn" onclick="ctp8VoicingReset(' + i + ')" style="width:100%;margin-top:4px;padding:3px 2px;font-size:.62rem;color:#a8acc0;">リセット</button>' +
        '</div>';
    }).join('');
  }
  window.ctp8VoicingOp = function (chordIdx, which) {
    var cur = ctp8GetOps(chordIdx);
    var next = cur.concat([which]).slice(-12);
    ctp8VoicingOps[chordIdx] = next;
    ctp8SaveShifts();
    ctp8RenderSequence(ctp8CurrentChordIdx);
    window.ctp8ApplyIfPlaying();
  };
  window.ctp8VoicingReset = function (chordIdx) {
    delete ctp8VoicingOps[chordIdx];
    ctp8SaveShifts();
    ctp8RenderSequence(ctp8CurrentChordIdx);
    window.ctp8ApplyIfPlaying();
  };
  window.ctp8Toggle = function () {
    ...
    if (!ctp8Engine) ctp8Engine = new ChordMetronomeEngineV2();
  ...
  window.ctp8ApplyVolume = function () {
    if (ctp8Engine) {
      ctp8Engine.metronomeGain = parseInt(document.getElementById('ctp8MetroVol').value, 10) / 100;
      ctp8Engine.chordGain = parseInt(document.getElementById('ctp8ChordVol').value, 10) / 100;
    }
  ...
  window.ctp8ApplyIfPlaying = function () {
    window.ctp8ApplyVolume();
    if (ctp8Engine && ctp8Engine.isPlaying) {
      ctp8Engine.stop();
      var btn = document.getElementById('ctp8StartBtn');
