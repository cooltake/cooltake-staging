...
<div class="ctp-card">
  ...
  <div class="ctp-pair">
    ...
    <div class="ctp-row">
      <label>演奏パターン</label>
      <select id="ctp4Pattern" class="ctp-select" onchange="ctp4ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      <label>メトロノーム音</label>
      <select id="ctp4MetroSound" class="ctp-select" onchange="ctp4ApplyIfPlaying()">
    ...
    <div class="ctp-row">
      ...
      <select id="ctp4Countdown" class="ctp-select" onchange="ctp4ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      ...
      <input id="ctp4MetroVol" type="range" min="0" max="100" value="80" oninput="ctp4ApplyVolume()">
    </div>
    <div class="ctp-row">
      <label>コード音量</label>
      <input id="ctp4ChordVol" type="range" min="0" max="100" value="80" oninput="ctp4ApplyVolume()">
    </div>
  </div>
...
function ctpActualChordName(chordDef, keyRoot, isMinor) {
  ...
  var ctp4PROGRESSION = { chords: [{deg:'II',q:'m7'},{deg:'V',q:'7'},{deg:'I',q:'maj7'},{deg:'VII',q:'m7b5'},{deg:'III',q:'7'}] };
  var ctp4Engine = null;
  var ctp4VoicingOps = {};
  ...
  function ctp4LoadShifts() {
    try {
      var raw = localStorage.getItem('cooltake_ctp4_voicing_ops_marusa');
      ctp4VoicingOps = raw ? JSON.parse(raw) : {};
    } catch (e) { ctp4VoicingOps = {}; }
  }
  function ctp4SaveShifts() {
    try { localStorage.setItem('cooltake_ctp4_voicing_ops_marusa', JSON.stringify(ctp4VoicingOps)); } catch (e) {}
  }
  function ctp4OpsKey(chordIdx) { return chordIdx; }
  function ctp4GetOps(chordIdx) { return ctp4VoicingOps[chordIdx] || []; }

  function ctp4ChordAvg(notes) { return notes.reduce(function(a,b){return a+b;}, 0) / notes.length; }
  ...
  function ctp4ClosestOctave(note, prevUpperNotes) {
    var MARGIN = 3;
  ...
  function ctp4VoiceLedBaseNotes(keyRootSemitone, isMinor) {
    var result = [];
    var prevUpper = null;
    ctp4PROGRESSION.chords.forEach(function (chordDef) {
      ...
      var bestUpper = upper.map(function (n) { return ctp4ClosestOctave(n, prevUpper); });
      result.push([bass].concat(bestUpper));
      prevUpper = bestUpper;
    });
    return result;
  }
  function ctp4ChordNotesFor(chordIdx) {
    var keyRoot = document.getElementById('ctp4KeyRoot').value;
    var keyMode = document.getElementById('ctp4KeyMode').value;
    var keyRootSemitone = NOTE_NAMES_V2.indexOf(keyRoot);
    var voiceLed = ctp4VoiceLedBaseNotes(keyRootSemitone, keyMode === 'minor');
    var baseNotes = voiceLed[chordIdx];
    var fixedBass = baseNotes.length > 0 ? baseNotes[0] : null;
    var notes = applyVoicingOps(baseNotes, ctp4GetOps(chordIdx));
  ...
  function ctp4GetOptions() {
    var idx;
    var octaveShifts = [];
    var voicingOps = ctp4PROGRESSION.chords.map(function (_, i) { return ctp4GetOps(i); });
    return {
      bpm: parseInt(document.getElementById('ctp4Bpm').value, 10),
      beatsPerBar: 4,
      timbre: document.getElementById('ctp4Timbre').value,
      pattern: document.getElementById('ctp4Pattern').value,
      metronomeSound: document.getElementById('ctp4MetroSound').value,
      progression: ctp4PROGRESSION,
      ...
      chordsOn: true,
      countdownBars: parseInt(document.getElementById('ctp4Countdown').value, 10),
      metronomeOn: true,
  ...
  function ctp4BuildDots() {
    var wrap = document.getElementById('ctp4BeatDots');
    wrap.innerHTML = Array.from({length: 4}, function (_, i) { return '<div class="ctp-dot" data-i="' + i + '"></div>'; }).join('');
  }
  function ctp4RenderSequence(activeIdx) {
    var keyRoot = document.getElementById('ctp4KeyRoot').value;
    var keyMode = document.getElementById('ctp4KeyMode').value;
    var wrap = document.getElementById('ctp4ProgressionSeq');
    wrap.innerHTML = ctp4PROGRESSION.chords.map(function (c, i) {
      var label = ctpChordLabel(c);
      var actualName = ctpActualChordName(c, keyRoot, keyMode === 'minor');
      var active = i === activeIdx;
      var r = ctp4ChordNotesFor(i);
      return '<div style="padding:8px 10px;border-radius:10px;min-width:120px;' +
        ...
        '<button class="rate-btn" onclick="ctp4VoicingOp(' + i + ',\'high\')" style="flex:1;padding:4px 2px;font-size:.65rem;" title="最高音を1オクターブ下げる">最高音↓</button>' +
        '<button class="rate-btn" onclick="ctp4VoicingOp(' + i + ',\'up\')" style="flex:1;padding:4px 2px;font-size:.65rem;" title="最低音を1オクターブ上げる">最低音↑</button>' +
        '</div>' +
        '<button class="rate-btn" onclick="ctp4VoicingReset(' + i + ')" style="width:100%;margin-top:4px;padding:3px 2px;font-size:.62rem;color:#a8acc0;">リセット</button>' +
        '</div>';
    }).join('');
  }
  window.ctp4VoicingOp = function (chordIdx, which) {
    var cur = ctp4GetOps(chordIdx);
    var next = cur.concat([which]).slice(-12);
    ctp4VoicingOps[chordIdx] = next;
    ctp4SaveShifts();
    ctp4RenderSequence(ctp4CurrentChordIdx);
    window.ctp4ApplyIfPlaying();
  };
  window.ctp4VoicingReset = function (chordIdx) {
    delete ctp4VoicingOps[chordIdx];
    ctp4SaveShifts();
    ctp4RenderSequence(ctp4CurrentChordIdx);
    window.ctp4ApplyIfPlaying();
  };
  window.ctp4Toggle = function () {
    var btn = document.getElementById('ctp4StartBtn');
    if (!ctp4Engine) ctp4Engine = new ChordMetronomeEngineV2();
    if (ctp4Engine.isPlaying) {
      ...
      ctp4Engine.onChordChange = function (idx) {
        ...
        ctp4RenderSequence(idx);
      };
      ctp4Engine.metronomeGain = parseInt(document.getElementById('ctp4MetroVol').value, 10) / 100;
      ctp4Engine.chordGain = parseInt(document.getElementById('ctp4ChordVol').value, 10) / 100;
      ctp4Engine.start(ctp4GetOptions());
      btn.textContent = '■ 停止';
    }
  };
  window.ctp4ApplyVolume = function () {
    if (ctp4Engine) {
      ctp4Engine.metronomeGain = parseInt(document.getElementById('ctp4MetroVol').value, 10) / 100;
      ctp4Engine.chordGain = parseInt(document.getElementById('ctp4ChordVol').value, 10) / 100;
    }
  ...
  window.ctp4ApplyIfPlaying = function () {
    window.ctp4ApplyVolume();
    if (ctp4Engine && ctp4Engine.isPlaying) {
      ctp4Engine.stop();
      var btn = document.getElementById('ctp4StartBtn');
