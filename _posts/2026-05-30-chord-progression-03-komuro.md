...
<div class="ctp-card">
  <div style="font-size:.85rem;font-weight:900;margin-bottom:12px;color:#e8e8ef;">▶ 小室進行をその場で聴く</div>

  <div class="ctp-row">
    <label>テンポ（BPM）</label>
    <div style="display:flex;align-items:center;gap:10px;">
      <input id="ctp3Bpm" type="range" min="40" max="240" value="90" oninput="ctp3Slide()" style="flex:1;">
      <div class="ctp-bpmnum" style="width:56px;"><span id="ctp3BpmNum">90</span></div>
    </div>
  </div>
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      ...
      <div style="display:flex;gap:6px;">
        <select id="ctp3KeyRoot" class="ctp-select" onchange="ctp3ApplyIfPlaying()">
          <option>C</option><option>C#</option><option>D</option><option>D#</option>
          <option>E</option><option>F</option><option>F#</option><option>G</option>
          <option>G#</option><option>A</option><option>A#</option><option>B</option>
        </select>
        <select id="ctp3KeyMode" class="ctp-select" onchange="ctp3ApplyIfPlaying()">
    ...
    <div class="ctp-row">
      <label>拍の長さ</label>
      <select id="ctp3ChordDur" class="ctp-select" onchange="ctp3ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      <label>音色</label>
      <select id="ctp3Timbre" class="ctp-select" onchange="ctp3ApplyIfPlaying()">
    ...
    <div class="ctp-row">
      <label>演奏パターン</label>
      <select id="ctp3Pattern" class="ctp-select" onchange="ctp3ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      <label>メトロノーム音</label>
      <select id="ctp3MetroSound" class="ctp-select" onchange="ctp3ApplyIfPlaying()">
    ...
    <div class="ctp-row">
      ...
      <select id="ctp3Countdown" class="ctp-select" onchange="ctp3ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      ...
      <input id="ctp3MetroVol" type="range" min="0" max="100" value="80" oninput="ctp3ApplyVolume()">
    </div>
    <div class="ctp-row">
      <label>コード音量</label>
      <input id="ctp3ChordVol" type="range" min="0" max="100" value="80" oninput="ctp3ApplyVolume()">
    </div>
  </div>

  <button class="ctp-btn" id="ctp3StartBtn" onclick="ctp3Toggle()">▶ 再生開始</button>
  <div class="ctp-dots" id="ctp3BeatDots"></div>
  <div id="ctp3StateLabel" style="font-size:.7rem;color:#a8acc0;text-align:center;margin-top:4px;"></div>

  <div id="ctp3ProgressionSeq" style="display:flex;flex-wrap:wrap;gap:10px;margin:12px 0;"></div>
...
function ctpActualChordName(chordDef, keyRoot, isMinor) {
  ...
  var ctp3PROGRESSION = { chords: [{deg:'VI',q:'m'},{deg:'IV',q:''},{deg:'I',q:''},{deg:'V',q:''}] };
  var ctp3Engine = null;
  var ctp3VoicingOps = {};
  var ctp3CurrentChordIdx = 0;

  function ctp3LoadShifts() {
    try {
      var raw = localStorage.getItem('cooltake_ctp3_voicing_ops_komuro');
      ctp3VoicingOps = raw ? JSON.parse(raw) : {};
    } catch (e) { ctp3VoicingOps = {}; }
  }
  function ctp3SaveShifts() {
    try { localStorage.setItem('cooltake_ctp3_voicing_ops_komuro', JSON.stringify(ctp3VoicingOps)); } catch (e) {}
  }
  function ctp3OpsKey(chordIdx) { return chordIdx; }
  function ctp3GetOps(chordIdx) { return ctp3VoicingOps[chordIdx] || []; }

  function ctp3ChordAvg(notes) { return notes.reduce(function(a,b){return a+b;}, 0) / notes.length; }
  ...
  function ctp3ClosestOctave(note, prevUpperNotes) {
    var MARGIN = 3;
  ...
  function ctp3VoiceLedBaseNotes(keyRootSemitone, isMinor) {
    var result = [];
    var prevUpper = null;
    ctp3PROGRESSION.chords.forEach(function (chordDef) {
      ...
      var bestUpper = upper.map(function (n) { return ctp3ClosestOctave(n, prevUpper); });
      result.push([bass].concat(bestUpper));
      prevUpper = bestUpper;
    });
    return result;
  }
  function ctp3ChordNotesFor(chordIdx) {
    var keyRoot = document.getElementById('ctp3KeyRoot').value;
    var keyMode = document.getElementById('ctp3KeyMode').value;
    var keyRootSemitone = NOTE_NAMES_V2.indexOf(keyRoot);
    var voiceLed = ctp3VoiceLedBaseNotes(keyRootSemitone, keyMode === 'minor');
    var baseNotes = voiceLed[chordIdx];
    var fixedBass = baseNotes.length > 0 ? baseNotes[0] : null;
    var notes = applyVoicingOps(baseNotes, ctp3GetOps(chordIdx));
    return { notes: notes, fixedBass: fixedBass };
  }

  window.ctp3Slide = function () {
    document.getElementById('ctp3BpmNum').textContent = document.getElementById('ctp3Bpm').value;
    window.ctp3ApplyIfPlaying();
  };
  function ctp3GetOptions() {
    var idx;
    var octaveShifts = [];
    var voicingOps = ctp3PROGRESSION.chords.map(function (_, i) { return ctp3GetOps(i); });
    return {
      bpm: parseInt(document.getElementById('ctp3Bpm').value, 10),
      beatsPerBar: 4,
      timbre: document.getElementById('ctp3Timbre').value,
      pattern: document.getElementById('ctp3Pattern').value,
      metronomeSound: document.getElementById('ctp3MetroSound').value,
      progression: ctp3PROGRESSION,
      keyRoot: document.getElementById('ctp3KeyRoot').value,
      keyMode: document.getElementById('ctp3KeyMode').value,
      chordDuration: document.getElementById('ctp3ChordDur').value,
      chordsOn: true,
      countdownBars: parseInt(document.getElementById('ctp3Countdown').value, 10),
      metronomeOn: true,
      octaveShifts: octaveShifts,
      voicingOps: voicingOps,
    };
  }
  function ctp3BuildDots() {
    var wrap = document.getElementById('ctp3BeatDots');
    wrap.innerHTML = Array.from({length: 4}, function (_, i) { return '<div class="ctp-dot" data-i="' + i + '"></div>'; }).join('');
  }
  function ctp3RenderSequence(activeIdx) {
    var keyRoot = document.getElementById('ctp3KeyRoot').value;
    var keyMode = document.getElementById('ctp3KeyMode').value;
    var wrap = document.getElementById('ctp3ProgressionSeq');
    wrap.innerHTML = ctp3PROGRESSION.chords.map(function (c, i) {
      var label = ctpChordLabel(c);
      var actualName = ctpActualChordName(c, keyRoot, keyMode === 'minor');
      var active = i === activeIdx;
      var r = ctp3ChordNotesFor(i);
      return '<div style="padding:8px 10px;border-radius:10px;min-width:120px;' +
        ...
        '<button class="rate-btn" onclick="ctp3VoicingOp(' + i + ',\'high\')" style="flex:1;padding:4px 2px;font-size:.65rem;" title="最高音を1オクターブ下げる">最高音↓</button>' +
        '<button class="rate-btn" onclick="ctp3VoicingOp(' + i + ',\'up\')" style="flex:1;padding:4px 2px;font-size:.65rem;" title="最低音を1オクターブ上げる">最低音↑</button>' +
        '</div>' +
        '<button class="rate-btn" onclick="ctp3VoicingReset(' + i + ')" style="width:100%;margin-top:4px;padding:3px 2px;font-size:.62rem;color:#a8acc0;">リセット</button>' +
        '</div>';
    }).join('');
  }
  window.ctp3VoicingOp = function (chordIdx, which) {
    var cur = ctp3GetOps(chordIdx);
    var next = cur.concat([which]).slice(-12);
    ctp3VoicingOps[chordIdx] = next;
    ctp3SaveShifts();
    ctp3RenderSequence(ctp3CurrentChordIdx);
    window.ctp3ApplyIfPlaying();
  };
  window.ctp3VoicingReset = function (chordIdx) {
    delete ctp3VoicingOps[chordIdx];
    ctp3SaveShifts();
    ctp3RenderSequence(ctp3CurrentChordIdx);
    window.ctp3ApplyIfPlaying();
  };
  window.ctp3Toggle = function () {
    var btn = document.getElementById('ctp3StartBtn');
    if (!ctp3Engine) ctp3Engine = new ChordMetronomeEngineV2();
    if (ctp3Engine.isPlaying) {
      ctp3Engine.stop();
      btn.textContent = '▶ 再生開始';
      document.getElementById('ctp3StateLabel').textContent = '';
      document.querySelectorAll('#ctp3BeatDots .ctp-dot').forEach(function (d) { d.style.background = '#383c4d'; });
    } else {
      ctp3BuildDots();
      ctp3Engine.onBeat = function (beatInBar, beatsPerBar, state) {
        document.querySelectorAll('#ctp3BeatDots .ctp-dot').forEach(function (d, i) {
          d.style.background = i === beatInBar ? (state === 'countdown' ? '#f59e0b' : '#8b9bff') : '#383c4d';
        });
        document.getElementById('ctp3StateLabel').textContent = state === 'countdown' ? 'カウント中…' : '';
      };
      ctp3Engine.onChordChange = function (idx) {
        ctp3CurrentChordIdx = idx;
        ctp3RenderSequence(idx);
      };
      ctp3Engine.metronomeGain = parseInt(document.getElementById('ctp3MetroVol').value, 10) / 100;
      ctp3Engine.chordGain = parseInt(document.getElementById('ctp3ChordVol').value, 10) / 100;
      ctp3Engine.start(ctp3GetOptions());
      btn.textContent = '■ 停止';
    }
  };
  window.ctp3ApplyVolume = function () {
    if (ctp3Engine) {
      ctp3Engine.metronomeGain = parseInt(document.getElementById('ctp3MetroVol').value, 10) / 100;
      ctp3Engine.chordGain = parseInt(document.getElementById('ctp3ChordVol').value, 10) / 100;
    }
  };
  window.ctp3ApplyIfPlaying = function () {
    window.ctp3ApplyVolume();
    if (ctp3Engine && ctp3Engine.isPlaying) {
      ctp3Engine.stop();
      var btn = document.getElementById('ctp3StartBtn');
