...
<div class="ctp-card">
  <div style="font-size:.85rem;font-weight:900;margin-bottom:12px;color:#e8e8ef;">▶ 切ない系進行をその場で聴く</div>

  <div class="ctp-row">
    <label>テンポ（BPM）</label>
    <div style="display:flex;align-items:center;gap:10px;">
      <input id="ctp6Bpm" type="range" min="40" max="240" value="90" oninput="ctp6Slide()" style="flex:1;">
      <div class="ctp-bpmnum" style="width:56px;"><span id="ctp6BpmNum">90</span></div>
    </div>
  </div>
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      ...
      <div style="display:flex;gap:6px;">
        <select id="ctp6KeyRoot" class="ctp-select" onchange="ctp6ApplyIfPlaying()">
          <option>C</option><option>C#</option><option>D</option><option>D#</option>
          <option>E</option><option>F</option><option>F#</option><option>G</option>
          <option>G#</option><option>A</option><option>A#</option><option>B</option>
        </select>
        <select id="ctp6KeyMode" class="ctp-select" onchange="ctp6ApplyIfPlaying()">
    ...
    <div class="ctp-row">
      <label>拍の長さ</label>
      <select id="ctp6ChordDur" class="ctp-select" onchange="ctp6ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      <label>音色</label>
      <select id="ctp6Timbre" class="ctp-select" onchange="ctp6ApplyIfPlaying()">
    ...
    <div class="ctp-row">
      <label>演奏パターン</label>
      <select id="ctp6Pattern" class="ctp-select" onchange="ctp6ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      <label>メトロノーム音</label>
      <select id="ctp6MetroSound" class="ctp-select" onchange="ctp6ApplyIfPlaying()">
    ...
    <div class="ctp-row">
      ...
      <select id="ctp6Countdown" class="ctp-select" onchange="ctp6ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      ...
      <input id="ctp6MetroVol" type="range" min="0" max="100" value="80" oninput="ctp6ApplyVolume()">
    </div>
    <div class="ctp-row">
      <label>コード音量</label>
      <input id="ctp6ChordVol" type="range" min="0" max="100" value="80" oninput="ctp6ApplyVolume()">
    </div>
  </div>

  <button class="ctp-btn" id="ctp6StartBtn" onclick="ctp6Toggle()">▶ 再生開始</button>
  <div class="ctp-dots" id="ctp6BeatDots"></div>
  <div id="ctp6StateLabel" style="font-size:.7rem;color:#a8acc0;text-align:center;margin-top:4px;"></div>

  <div id="ctp6ProgressionSeq" style="display:flex;flex-wrap:wrap;gap:10px;margin:12px 0;"></div>
...
function ctpActualChordName(chordDef, keyRoot, isMinor) {
  ...
  var ctp6PROGRESSION = { chords: [{deg:'I',q:''},{deg:'V',q:''},{deg:'VI',q:'m'},{deg:'III',q:'m'},{deg:'IV',q:''},{deg:'I',q:''},{deg:'IV',q:''},{deg:'V',q:''}] };
  var ctp6Engine = null;
  var ctp6VoicingOps = {};
  var ctp6CurrentChordIdx = 0;

  function ctp6LoadShifts() {
    try {
      var raw = localStorage.getItem('cooltake_ctp6_voicing_ops_setsunai');
      ctp6VoicingOps = raw ? JSON.parse(raw) : {};
    } catch (e) { ctp6VoicingOps = {}; }
  }
  function ctp6SaveShifts() {
    try { localStorage.setItem('cooltake_ctp6_voicing_ops_setsunai', JSON.stringify(ctp6VoicingOps)); } catch (e) {}
  }
  function ctp6OpsKey(chordIdx) { return chordIdx; }
  function ctp6GetOps(chordIdx) { return ctp6VoicingOps[chordIdx] || []; }

  function ctp6ChordAvg(notes) { return notes.reduce(function(a,b){return a+b;}, 0) / notes.length; }
  ...
  function ctp6ClosestOctave(note, prevUpperNotes) {
    var MARGIN = 3;
  ...
  function ctp6VoiceLedBaseNotes(keyRootSemitone, isMinor) {
    var result = [];
    var prevUpper = null;
    ctp6PROGRESSION.chords.forEach(function (chordDef) {
      ...
      var bestUpper = upper.map(function (n) { return ctp6ClosestOctave(n, prevUpper); });
      result.push([bass].concat(bestUpper));
      prevUpper = bestUpper;
    });
    return result;
  }
  function ctp6ChordNotesFor(chordIdx) {
    var keyRoot = document.getElementById('ctp6KeyRoot').value;
    var keyMode = document.getElementById('ctp6KeyMode').value;
    var keyRootSemitone = NOTE_NAMES_V2.indexOf(keyRoot);
    var voiceLed = ctp6VoiceLedBaseNotes(keyRootSemitone, keyMode === 'minor');
    var baseNotes = voiceLed[chordIdx];
    var fixedBass = baseNotes.length > 0 ? baseNotes[0] : null;
    var notes = applyVoicingOps(baseNotes, ctp6GetOps(chordIdx));
    return { notes: notes, fixedBass: fixedBass };
  }

  window.ctp6Slide = function () {
    document.getElementById('ctp6BpmNum').textContent = document.getElementById('ctp6Bpm').value;
    window.ctp6ApplyIfPlaying();
  };
  function ctp6GetOptions() {
    var idx;
    var octaveShifts = [];
    var voicingOps = ctp6PROGRESSION.chords.map(function (_, i) { return ctp6GetOps(i); });
    return {
      bpm: parseInt(document.getElementById('ctp6Bpm').value, 10),
      beatsPerBar: 4,
      timbre: document.getElementById('ctp6Timbre').value,
      pattern: document.getElementById('ctp6Pattern').value,
      metronomeSound: document.getElementById('ctp6MetroSound').value,
      progression: ctp6PROGRESSION,
      keyRoot: document.getElementById('ctp6KeyRoot').value,
      keyMode: document.getElementById('ctp6KeyMode').value,
      chordDuration: document.getElementById('ctp6ChordDur').value,
      chordsOn: true,
      countdownBars: parseInt(document.getElementById('ctp6Countdown').value, 10),
      metronomeOn: true,
      octaveShifts: octaveShifts,
      voicingOps: voicingOps,
    };
  }
  function ctp6BuildDots() {
    var wrap = document.getElementById('ctp6BeatDots');
    wrap.innerHTML = Array.from({length: 4}, function (_, i) { return '<div class="ctp-dot" data-i="' + i + '"></div>'; }).join('');
  }
  function ctp6RenderSequence(activeIdx) {
    var keyRoot = document.getElementById('ctp6KeyRoot').value;
    var keyMode = document.getElementById('ctp6KeyMode').value;
    var wrap = document.getElementById('ctp6ProgressionSeq');
    wrap.innerHTML = ctp6PROGRESSION.chords.map(function (c, i) {
      var label = ctpChordLabel(c);
      var actualName = ctpActualChordName(c, keyRoot, keyMode === 'minor');
      var active = i === activeIdx;
      var r = ctp6ChordNotesFor(i);
      return '<div style="padding:8px 10px;border-radius:10px;min-width:120px;' +
        ...
        '<button class="rate-btn" onclick="ctp6VoicingOp(' + i + ',\'high\')" style="flex:1;padding:4px 2px;font-size:.65rem;" title="最高音を1オクターブ下げる">最高音↓</button>' +
        '<button class="rate-btn" onclick="ctp6VoicingOp(' + i + ',\'up\')" style="flex:1;padding:4px 2px;font-size:.65rem;" title="最低音を1オクターブ上げる">最低音↑</button>' +
        '</div>' +
        '<button class="rate-btn" onclick="ctp6VoicingReset(' + i + ')" style="width:100%;margin-top:4px;padding:3px 2px;font-size:.62rem;color:#a8acc0;">リセット</button>' +
        '</div>';
    }).join('');
  }
  window.ctp6VoicingOp = function (chordIdx, which) {
    var cur = ctp6GetOps(chordIdx);
    var next = cur.concat([which]).slice(-12);
    ctp6VoicingOps[chordIdx] = next;
    ctp6SaveShifts();
    ctp6RenderSequence(ctp6CurrentChordIdx);
    window.ctp6ApplyIfPlaying();
  };
  window.ctp6VoicingReset = function (chordIdx) {
    delete ctp6VoicingOps[chordIdx];
    ctp6SaveShifts();
    ctp6RenderSequence(ctp6CurrentChordIdx);
    window.ctp6ApplyIfPlaying();
  };
  window.ctp6Toggle = function () {
    var btn = document.getElementById('ctp6StartBtn');
    if (!ctp6Engine) ctp6Engine = new ChordMetronomeEngineV2();
    if (ctp6Engine.isPlaying) {
      ctp6Engine.stop();
      btn.textContent = '▶ 再生開始';
      document.getElementById('ctp6StateLabel').textContent = '';
      document.querySelectorAll('#ctp6BeatDots .ctp-dot').forEach(function (d) { d.style.background = '#383c4d'; });
    } else {
      ctp6BuildDots();
      ctp6Engine.onBeat = function (beatInBar, beatsPerBar, state) {
        document.querySelectorAll('#ctp6BeatDots .ctp-dot').forEach(function (d, i) {
          d.style.background = i === beatInBar ? (state === 'countdown' ? '#f59e0b' : '#8b9bff') : '#383c4d';
        });
        document.getElementById('ctp6StateLabel').textContent = state === 'countdown' ? 'カウント中…' : '';
      };
      ctp6Engine.onChordChange = function (idx) {
        ctp6CurrentChordIdx = idx;
        ctp6RenderSequence(idx);
      };
      ctp6Engine.metronomeGain = parseInt(document.getElementById('ctp6MetroVol').value, 10) / 100;
      ctp6Engine.chordGain = parseInt(document.getElementById('ctp6ChordVol').value, 10) / 100;
      ctp6Engine.start(ctp6GetOptions());
      btn.textContent = '■ 停止';
    }
  };
  window.ctp6ApplyVolume = function () {
    if (ctp6Engine) {
      ctp6Engine.metronomeGain = parseInt(document.getElementById('ctp6MetroVol').value, 10) / 100;
      ctp6Engine.chordGain = parseInt(document.getElementById('ctp6ChordVol').value, 10) / 100;
    }
  };
  window.ctp6ApplyIfPlaying = function () {
    window.ctp6ApplyVolume();
    if (ctp6Engine && ctp6Engine.isPlaying) {
      ctp6Engine.stop();
      var btn = document.getElementById('ctp6StartBtn');
