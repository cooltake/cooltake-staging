...
<div class="ctp-card">
  <div style="font-size:.85rem;font-weight:900;margin-bottom:12px;color:#e8e8ef;">▶ カノン進行をその場で聴く</div>

  <div class="ctp-row">
    <label>テンポ（BPM）</label>
    <div style="display:flex;align-items:center;gap:10px;">
      <input id="ctp2Bpm" type="range" min="40" max="240" value="90" oninput="ctp2Slide()" style="flex:1;">
      <div class="ctp-bpmnum" style="width:56px;"><span id="ctp2BpmNum">90</span></div>
    </div>
  </div>
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      ...
      <div style="display:flex;gap:6px;">
        <select id="ctp2KeyRoot" class="ctp-select" onchange="ctp2ApplyIfPlaying()">
          <option>C</option><option>C#</option><option>D</option><option>D#</option>
          <option>E</option><option>F</option><option>F#</option><option>G</option>
          <option>G#</option><option>A</option><option>A#</option><option>B</option>
        </select>
        <select id="ctp2KeyMode" class="ctp-select" onchange="ctp2ApplyIfPlaying()">
    ...
    <div class="ctp-row">
      <label>拍の長さ</label>
      <select id="ctp2ChordDur" class="ctp-select" onchange="ctp2ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      <label>音色</label>
      <select id="ctp2Timbre" class="ctp-select" onchange="ctp2ApplyIfPlaying()">
    ...
    <div class="ctp-row">
      <label>演奏パターン</label>
      <select id="ctp2Pattern" class="ctp-select" onchange="ctp2ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      <label>メトロノーム音</label>
      <select id="ctp2MetroSound" class="ctp-select" onchange="ctp2ApplyIfPlaying()">
    ...
    <div class="ctp-row">
      ...
      <select id="ctp2Countdown" class="ctp-select" onchange="ctp2ApplyIfPlaying()">
  ...
  <div class="ctp-pair">
    <div class="ctp-row">
      ...
      <input id="ctp2MetroVol" type="range" min="0" max="100" value="80" oninput="ctp2ApplyVolume()">
    </div>
    <div class="ctp-row">
      <label>コード音量</label>
      <input id="ctp2ChordVol" type="range" min="0" max="100" value="80" oninput="ctp2ApplyVolume()">
    </div>
  </div>

  <button class="ctp-btn" id="ctp2StartBtn" onclick="ctp2Toggle()">▶ 再生開始</button>
  <div class="ctp-dots" id="ctp2BeatDots"></div>
  <div id="ctp2StateLabel" style="font-size:.7rem;color:#a8acc0;text-align:center;margin-top:4px;"></div>

  <div id="ctp2ProgressionSeq" style="display:flex;flex-wrap:wrap;gap:10px;margin:12px 0;"></div>
...
function ctpActualChordName(chordDef, keyRoot, isMinor) {
  ...
  var ctp2PROGRESSION = { chords: [{deg:'I',q:''},{deg:'V',q:''},{deg:'VI',q:'m'},{deg:'III',q:'m'},{deg:'IV',q:''},{deg:'I',q:''},{deg:'IV',q:''},{deg:'V',q:''}] };
  var ctp2Engine = null;
  var ctp2VoicingOps = {};
  var ctp2CurrentChordIdx = 0;

  function ctp2LoadShifts() {
    try {
      var raw = localStorage.getItem('cooltake_ctp2_voicing_ops_canon');
      ctp2VoicingOps = raw ? JSON.parse(raw) : {};
    } catch (e) { ctp2VoicingOps = {}; }
  }
  function ctp2SaveShifts() {
    try { localStorage.setItem('cooltake_ctp2_voicing_ops_canon', JSON.stringify(ctp2VoicingOps)); } catch (e) {}
  }
  function ctp2OpsKey(chordIdx) { return chordIdx; }
  function ctp2GetOps(chordIdx) { return ctp2VoicingOps[chordIdx] || []; }

  function ctp2ChordAvg(notes) { return notes.reduce(function(a,b){return a+b;}, 0) / notes.length; }
  ...
  function ctp2ClosestOctave(note, prevUpperNotes) {
    var MARGIN = 3;
  ...
  function ctp2VoiceLedBaseNotes(keyRootSemitone, isMinor) {
    var result = [];
    var prevUpper = null;
    ctp2PROGRESSION.chords.forEach(function (chordDef) {
      ...
      var bestUpper = upper.map(function (n) { return ctp2ClosestOctave(n, prevUpper); });
      result.push([bass].concat(bestUpper));
      prevUpper = bestUpper;
    });
    return result;
  }
  function ctp2ChordNotesFor(chordIdx) {
    var keyRoot = document.getElementById('ctp2KeyRoot').value;
    var keyMode = document.getElementById('ctp2KeyMode').value;
    var keyRootSemitone = NOTE_NAMES_V2.indexOf(keyRoot);
    var voiceLed = ctp2VoiceLedBaseNotes(keyRootSemitone, keyMode === 'minor');
    var baseNotes = voiceLed[chordIdx];
    var fixedBass = baseNotes.length > 0 ? baseNotes[0] : null;
    var notes = applyVoicingOps(baseNotes, ctp2GetOps(chordIdx));
    return { notes: notes, fixedBass: fixedBass };
  }

  window.ctp2Slide = function () {
    document.getElementById('ctp2BpmNum').textContent = document.getElementById('ctp2Bpm').value;
    window.ctp2ApplyIfPlaying();
  };
  function ctp2GetOptions() {
    var idx;
    var octaveShifts = [];
    var voicingOps = ctp2PROGRESSION.chords.map(function (_, i) { return ctp2GetOps(i); });
    return {
      bpm: parseInt(document.getElementById('ctp2Bpm').value, 10),
      beatsPerBar: 4,
      timbre: document.getElementById('ctp2Timbre').value,
      pattern: document.getElementById('ctp2Pattern').value,
      metronomeSound: document.getElementById('ctp2MetroSound').value,
      progression: ctp2PROGRESSION,
      keyRoot: document.getElementById('ctp2KeyRoot').value,
      keyMode: document.getElementById('ctp2KeyMode').value,
      chordDuration: document.getElementById('ctp2ChordDur').value,
      chordsOn: true,
      countdownBars: parseInt(document.getElementById('ctp2Countdown').value, 10),
      metronomeOn: true,
      octaveShifts: octaveShifts,
      voicingOps: voicingOps,
    };
  }
  function ctp2BuildDots() {
    var wrap = document.getElementById('ctp2BeatDots');
    wrap.innerHTML = Array.from({length: 4}, function (_, i) { return '<div class="ctp-dot" data-i="' + i + '"></div>'; }).join('');
  }
  function ctp2RenderSequence(activeIdx) {
    var keyRoot = document.getElementById('ctp2KeyRoot').value;
    var keyMode = document.getElementById('ctp2KeyMode').value;
    var wrap = document.getElementById('ctp2ProgressionSeq');
    wrap.innerHTML = ctp2PROGRESSION.chords.map(function (c, i) {
      var label = ctpChordLabel(c);
      var actualName = ctpActualChordName(c, keyRoot, keyMode === 'minor');
      var active = i === activeIdx;
      var r = ctp2ChordNotesFor(i);
      return '<div style="padding:8px 10px;border-radius:10px;min-width:120px;' +
        ...
        '<button class="rate-btn" onclick="ctp2VoicingOp(' + i + ',\'high\')" style="flex:1;padding:4px 2px;font-size:.65rem;" title="最高音を1オクターブ下げる">最高音↓</button>' +
        '<button class="rate-btn" onclick="ctp2VoicingOp(' + i + ',\'up\')" style="flex:1;padding:4px 2px;font-size:.65rem;" title="最低音を1オクターブ上げる">最低音↑</button>' +
        '</div>' +
        '<button class="rate-btn" onclick="ctp2VoicingReset(' + i + ')" style="width:100%;margin-top:4px;padding:3px 2px;font-size:.62rem;color:#a8acc0;">リセット</button>' +
        '</div>';
    }).join('');
  }
  window.ctp2VoicingOp = function (chordIdx, which) {
    var cur = ctp2GetOps(chordIdx);
    var next = cur.concat([which]).slice(-12);
    ctp2VoicingOps[chordIdx] = next;
    ctp2SaveShifts();
    ctp2RenderSequence(ctp2CurrentChordIdx);
    window.ctp2ApplyIfPlaying();
  };
  window.ctp2VoicingReset = function (chordIdx) {
    delete ctp2VoicingOps[chordIdx];
    ctp2SaveShifts();
    ctp2RenderSequence(ctp2CurrentChordIdx);
    window.ctp2ApplyIfPlaying();
  };
  window.ctp2Toggle = function () {
    var btn = document.getElementById('ctp2StartBtn');
    if (!ctp2Engine) ctp2Engine = new ChordMetronomeEngineV2();
    if (ctp2Engine.isPlaying) {
      ctp2Engine.stop();
      btn.textContent = '▶ 再生開始';
      document.getElementById('ctp2StateLabel').textContent = '';
      document.querySelectorAll('#ctp2BeatDots .ctp-dot').forEach(function (d) { d.style.background = '#383c4d'; });
    } else {
      ctp2BuildDots();
      ctp2Engine.onBeat = function (beatInBar, beatsPerBar, state) {
        document.querySelectorAll('#ctp2BeatDots .ctp-dot').forEach(function (d, i) {
          d.style.background = i === beatInBar ? (state === 'countdown' ? '#f59e0b' : '#8b9bff') : '#383c4d';
        });
        document.getElementById('ctp2StateLabel').textContent = state === 'countdown' ? 'カウント中…' : '';
      };
      ctp2Engine.onChordChange = function (idx) {
        ctp2CurrentChordIdx = idx;
        ctp2RenderSequence(idx);
      };
      ctp2Engine.metronomeGain = parseInt(document.getElementById('ctp2MetroVol').value, 10) / 100;
      ctp2Engine.chordGain = parseInt(document.getElementById('ctp2ChordVol').value, 10) / 100;
      ctp2Engine.start(ctp2GetOptions());
      btn.textContent = '■ 停止';
    }
  };
  window.ctp2ApplyVolume = function () {
    if (ctp2Engine) {
      ctp2Engine.metronomeGain = parseInt(document.getElementById('ctp2MetroVol').value, 10) / 100;
      ctp2Engine.chordGain = parseInt(document.getElementById('ctp2ChordVol').value, 10) / 100;
    }
  };
  window.ctp2ApplyIfPlaying = function () {
    window.ctp2ApplyVolume();
    if (ctp2Engine && ctp2Engine.isPlaying) {
      ctp2Engine.stop();
      var btn = document.getElementById('ctp2StartBtn');
